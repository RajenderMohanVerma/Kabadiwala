...
function Table({ rows, columns, empty = 'New records will appear as the chain progresses.' }) {
  ...
  return <div className="panel table-wrap"><table><thead><tr>{columns.map(([key, title]) => <th key={key}>{title}</th>)}</tr></thead><tbody>
    {rows.map((row, index) => <tr key={row.id || row.batchCode || row.pickupCode || `${row.createdAt || 'row'}-${index}`}>
      {columns.map(([key, _title, render]) => <td key={`${row.id || index}-${key}`}>{render ? render(row) : key === 'status' ? <Status value={valueAt(row, key)} /> : key === 'images' || key.endsWith('.images') ? <ImageThumbs images={valueAt(row, key) || []} /> : String(valueAt(row, key) ?? '—')}</td>)}
    </tr>)}
  </tbody></table></div>
}
...
export function HubBatchPage() {
  ...
  return <Page title={batch?.batchCode || 'Batch details'} copy="Review the chain of custody before handoff."><State state={state} />{batch && <><div className="stat-grid"><div className="stat-card"><span>Weight</span><strong>{batch.totalWeight} kg</strong></div><div className="stat-card"><span>Status</span><strong>{labels(batch.status)}</strong></div><div className="stat-card"><span>Recycler</span><strong>{batch.recycler?.name || 'Not assigned'}</strong></div></div><Table rows={batch.pickups} columns={[['pickup.pickupCode', 'Pickup'], ['pickup.category', 'Category'], ['pickup.images', 'Item photos'], ['weight', 'Weight (kg)']]} /><Table rows={batch.events} columns={[['status', 'Event'], ['actor.name', 'Actor'], ['createdAt', 'When']]} /></>}</Page>
}
...
function AdminUsers({ kind }) {
  ...
  const selectedUser = detailState.data?.user
  const detailValue = (value) => value || '—'
  return <Page title={`Admin ${labels(kind)}`} copy="Manage verification, access and operational role records."><State state={state} /><ActionMessage action={action} />{kind === 'users' && selectedUser && <section className="panel user-detail" aria-label={`Details for ${selectedUser.name}`}><div className="panel-heading"><h2>User details</h2><button type="button" className="button secondary small" onClick={() => setSelectedUserId('')}>Close</button></div><dl className="details"><dt>Name</dt><dd>{detailValue(selectedUser.name)}</dd><dt>Email</dt><dd>{detailValue(selectedUser.email)}</dd><dt>Phone</dt><dd>{detailValue(selectedUser.phone)}</dd><dt>Role</dt><dd>{labels(selectedUser.role)}</dd><dt>Status</dt><dd>{labels(selectedUser.status)}</dd><dt>Verified</dt><dd>{selectedUser.verified ? 'Yes' : 'No'}</dd><dt>Address</dt><dd>{[selectedUser.address, selectedUser.city, selectedUser.state, selectedUser.pincode].filter(Boolean).join(', ') || '—'}</dd><dt>Bio</dt><dd>{detailValue(selectedUser.bio)}</dd><dt>Rating</dt><dd>{selectedUser.rating} ({selectedUser.totalRatings} ratings)</dd><dt>Availability</dt><dd>{selectedUser.available ? 'Available' : 'Unavailable'}</dd><dt>Service area</dt><dd>{detailValue(selectedUser.serviceArea)}</dd><dt>Supported categories</dt><dd>{detailValue(selectedUser.supportedCategories)}</dd><dt>Capacity</dt><dd>{selectedUser.capacityKg ? `${selectedUser.capacityKg} kg` : '—'}</dd><dt>Joined</dt><dd>{new Date(selectedUser.createdAt).toLocaleString()}</dd><dt>Pickups</dt><dd>{selectedUser._count.customerPickups + selectedUser._count.collectorPickups}</dd><dt>Reviews</dt><dd>{selectedUser._count.reviewsGiven} given, {selectedUser._count.reviewsReceived} received</dd><dt>Complaints</dt><dd>{selectedUser._count.complaints}</dd></dl></section>}{kind === 'users' && selectedUserId && detailState.loading && <LoadingState />}{kind === 'users' && selectedUserId && detailState.error && <ErrorState message={detailState.error} />}<Table rows={users} columns={[['name', 'Name'], ['email', 'Email'], ['role', 'Role'], ['status', 'Status'], ['verified', 'Verified'], ['actions', 'Actions', (user) => <div className="row-actions">{kind === 'users' && <button className="button secondary small" onClick={() => setSelectedUserId(user.id)}>View details</button>}{user.role !== 'CUSTOMER' && (!user.verified || user.status !== 'ACTIVE') && <button className="button primary small" onClick={() => update(user, { verified: true, status: 'ACTIVE' })}>Approve</button>}<button className="button secondary small" onClick={() => update(user, { verified: !user.verified })}>{user.verified ? 'Unverify' : 'Verify'}</button><select aria-label={`Status for ${user.email}`} value={user.status} onChange={(event) => update(user, { status: event.target.value })}><option>ACTIVE</option><option>INACTIVE</option><option>SUSPENDED</option></select></div>]]} /></Page>
}
